...
# Usamos um try-except para lidar com a diferença entre execução em notebook e como arquivo .py.
# Quando executado como arquivo .py (no Streamlit Cloud), __file__ estará disponível.
# Em um notebook, __file__ não está definido, então usamos um caminho absoluto temporário.
try:
    # Caminho relativo quando executado como um arquivo .py (ideal para Streamlit Cloud)
    # O app.py e model_logic.py estarão na raiz, e os artefatos na subpasta 'data'.
    ARTIFACTS_BASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data')
    MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'final_model.pkl')
except NameError:
    # Caminho absoluto para teste em ambiente de notebook (Colab)
    # Ajuste este caminho se o seu diretório 'model_artifacts' não estiver aqui.
    ARTIFACTS_BASE_PATH = '/content/drive/MyDrive/UFPE_Rankings/model_artifacts/data'
    MODEL_PATH = '/content/drive/MyDrive/UFPE_Rankings/model_artifacts/final_model.pkl'
